...
class MyStudyGalleryFragmentRVAdapter(private val dataList: List<ImageResponse.ImagesInfo>) : RecyclerView.Adapter<MyStudyGalleryFragmentRVAdapter.ViewHolder>() {

    interface ItemClick {
        fun onItemClick(data : ImageResponse.ImagesInfo)
    }
    ...
    inner class ViewHolder(private val binding: ItemGalleryPictureBinding) : RecyclerView.ViewHolder(binding.root) {
        fun bind(data: ImageResponse.ImagesInfo) {
            // Glide를 사용해 이미지를 로드
